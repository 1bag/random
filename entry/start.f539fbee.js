import { o as me, t as we } from "../chunks/index.4c78ea70.js";
import { S as Ge, a as Je, I as M, g as Ce, f as De, b as ye, c as le, s as te, i as _e, d as B, o as Fe, P as Me, e as Ze } from "../chunks/singletons.d8eea783.js";
import { b as q } from "../chunks/paths.6c27e217.js";
function Qe(t, r) {
...
window.fetch = (t, r) => ((t instanceof Request ? t.method : (r == null ? void 0 : r.method) || "GET") !== "GET" && ae.delete(Se(t)),
    Ke(t, r));
const ae = new Map;
function ct(t) {
    const r = atob(t)
...
function lt(t, r) {
    ...
    if (i != null && i.textContent) {
        let { body: p, ...u } = JSON.parse(i.textContent);
        ...
        return E && ae.set(f, {
            ...
            i.getAttribute("data-b64") !== null && (p = ct(p)),
            Promise.resolve(new Response(p, u))
    }
...
function ft(t, r, f) {
    if (ae.size > 0) {
        const i = Se(t, f)
            , p = ae.get(i);
        if (p) {
            if (performance.now() < p.ttl && ["default", "force-cache", "only-if-cached", void 0].includes(f == null ? void 0 : f.cache))
...
function Se(t, r) {
    let i = `script[data-sveltekit-fetched][data-url=${JSON.stringify(tinstanceof Request ? t.url : t)}]`;
...
function dt(t) {
    const r = [];
    return {
        pattern: t === "/" ? /^\/$/ : new RegExp(`^${ht(t).map(i => {
            const p = /^\[\.\.\.(\w+)(?:=(\w+))?\]$/.exec(i);
            if (p)
                return r.push({
                    name: p[1],
            ...
            return "/" + E.map((g, m) => {
                if (m % 2) {
                    if (g.startsWith("x+"))
                        return ve(String.fromCharCode(parseInt(g.slice(2), 16)));
                    if (g.startsWith("u+"))
                        return ve(String.fromCharCode(...g.slice(2).split("-").map(U => parseInt(U, 16))));
                    const d = ut.exec(g);
                    if (!d)
                        throw new Error(`Invalid param: ${g}. Params and matcher names can only have underscores and alphanumeric characters.`);
                    const [, N, T, R, D] = d;
                    return r.push({
                        name: R,
                        matcher: D,
...
function ht(t) {
    return t.slice(1).split("/").filter(pt)
}
function gt(t, r, f) {
    const i = {}
        , p = t.slice(1)
        , u = p.filter(l => l !== void 0);
    let E = 0;
    for (let l = 0; l < r.length; l += 1) {
        const g = r[l];
        let m = p[l - E];
        if (g.chained && g.rest && E && (m = p.slice(l - E, l + 1).filter(d => d).join("/"),
            E = 0),
            m === void 0) {
            g.rest && (i[g.name] = "");
            continue
...
function mt({ nodes: t, server_loads: r, dictionary: f, matchers: i }) {
    ...
    return Object.entries(f).map(([l, [g, m, d]]) => {
        ...
        return R.errors.length = R.layouts.length = Math.max(R.errors.length, R.layouts.length),
            R
    }
    );
    function u(l) {
...
async function At(t, r) {
    ...
    for (const i in t)
        if (typeof ((f = t[i]) == null ? void 0 : f.then) == "function")
            return Object.fromEntries(await Promise.all(Object.entries(t).map(async ([p, u]) => [p, await u])));
    return t
}
class ne {
    constructor(r, f) {
        this.status = r,
            typeof f == "string" ? this.body = {
                message: f
...
function Ot(t, r) {
    ...
    let d = {
        ...
        error: null,
        url: null
    }, N = !1, T = !1, R = !0, D = !1, U = !1, C = !1, z = !1, V, x = (je = history.state) == null ? void 0 : je[M];
    x || (x = Date.now(),
    ...
    async function re(e, { noScroll: s = !1, replaceState: n = !1, keepFocus: o = !1, state: a = {}, invalidateAll: c = !1 }, h, _) {
        return typeof e == "string" && (e = new URL(e, Ce(document))),
            ce({
                url: e,
                scroll: s ? te() : null,
                keepfocus: o,
    ...
    async function Le(e) {
        return g = {
            id: e.id,
            promise: pe(e).then(s => (s.type === "loaded" && s.state.error && (g = null),
                s))
        },
            g.promise
    }
    ...
    function Pe(e) {
        ...
        m.after_navigate.forEach(a => a(n)),
            T = !0
    }
    async function X({ url: e, params: s, branch: n, status: o, error: a, route: c, form: h }) {
        ...
        return (!d.url || e.href !== d.url.href || d.error !== a || h !== void 0 && h !== H.form || L) && (b.props.page = {
            error: a,
            params: s,
            route: {
    ...
    async function ue({ loader: e, parent: s, url: n, params: o, route: a, server_data_node: c }) {
        ...
        if ((v = b.universal) != null && v.load) {
            ...
            h = await b.universal.load.call(null, y) ?? null,
                h = h ? await At(h, a.id) : null
        }
        return {
            node: b,
            loader: e,
    ...
    function Oe(e, s, n, o, a) {
        ...
        if (!o)
            return !1;
        if (o.parent && e || o.route && s || o.url && n)
            return !0;
        for (const c of o.params)
            if (a[c] !== d.params[c])
                return !0;
        for (const c of o.dependencies)
            if (E.some(h => h(new URL(c))))
                return !0;
        return !1
    }
    function de(e, s) {
    ...
    async function pe({ id: e, invalidating: s, url: n, params: o, route: a }) {
        if ((g == null ? void 0 : g.id) === e)
            return g.promise;
        const { errors: c, layouts: h, leaf: _ } = a
            , b = [...h, _];
        ...
        const j = b.map(async (S, I) => {
            ...
            const k = d.branch[I]
                , $ = w == null ? void 0 : w[I];
            if ((!$ || $.type === "skip") && S[1] === (k == null ? void 0 : k.loader) && !Oe(O, A, L, (he = k.universal) == null ? void 0 : he.uses, o))
                return k;
            if (O = !0,
    ...
    function Ue({ url: e, type: s, intent: n, delta: o }) {
        ...
        const h = {
            ...
            cancel: () => {
                a = !0,
                    c.reject(new Error("navigation was cancelled"))
            }
        };
        return U || m.before_navigate.forEach(_ => _(h)),
            a ? null : c
    }
    async function ce({ url: e, scroll: s, keepfocus: n, redirect_count: o, details: a, type: c, delta: h, nav_token: _ = {}, accepted: b, blocked: v }) {
        var j, F, S;
        const L = Z(e, !1)
            , A = Ue({
                url: e,
        ...
        b(),
            U = !0,
            T && B.navigating.set(A.navigation),
            W = _;
        let y = L && await pe(L);
        if (!y) {
            if (_e(e, q))
        ...
        if (e = (L == null ? void 0 : L.url) || e,
            W !== _)
            return A.reject(new Error("navigation was aborted")),
                !1;
        if (y.type === "redirect")
            if (o >= 20)
                y = await ie({
                ...
                return re(new URL(y.location, e).href, {}, o + 1, _),
                    !1;
        else
            ((j = y.props.page) == null ? void 0 : j.status) >= 400 && await B.updated.check() && await K(e);
        if (E.length = 0,
            z = !1,
            D = !0,
            be(P),
            Re(P),
            (F = y.props.page) != null && F.url && y.props.page.url.pathname !== e.pathname && (e.pathname = (S = y.props.page) == null ? void 0 : S.url.pathname),
            a) {
        ...
        if (g = null,
            T) {
            d = y.state,
                y.props.page && (y.props.page.url = e);
    ...
    function Xe() {
        ...
        const n = new IntersectionObserver(c => {
            for (const h of c)
                h.isIntersecting && (oe(se(new URL(h.target.href))),
                    n.unobserve(h.target))
        }
            , {
                threshold: 0
            });
        function o(c, h) {
            ...
            if (!A.reload)
                if (h <= A.preload_data) {
                    ...
                    h <= A.preload_code && oe(se(b))
        }
        function a() {
            n.disconnect();
            for (const c of u.querySelectorAll("a")) {
                const { url: h, external: _, download: b } = ye(c, q);
                if (_ || b)
                    continue;
                const v = le(c);
                v.reload || (v.preload_code === Me.viewport && n.observe(c),
                    v.preload_code === Me.eager && oe(se(h)))
            }
        }
        m.after_navigate.push(a),
            a()
    }
    function Q(e, s) {
    ...
    return {
        ...
        _start_router: () => {
            ...
            history.scrollRestoration = "manual",
                ...
                u.addEventListener("click", n => {
                    ...
                    if (!o)
                        return;
                    const { url: a, external: c, target: h, download: _ } = ye(o, q);
                    if (!a)
                        return;
                    if (h === "_parent" || h === "_top") {
                        ...
                        return;
                    const b = le(o);
                    if (!(o instanceof SVGAElement) && a.protocol !== location.protocol && !(a.protocol === "https:" || a.protocol === "http:") || _)
                        return;
                    if (c || b.reload) {
                        Ue({
                            url: a,
                    ...
                    if (A !== void 0 && L === location.href.split("#")[0]) {
                        if (d.url.hash === a.hash) {
                            n.preventDefault(),
                                (P = o.ownerDocument.getElementById(A)) == null || P.scrollIntoView();
                            return
                        }
                        if (C = !0,
                            be(x),
                ...
                u.addEventListener("submit", n => {
                    ...
                    if (((a == null ? void 0 : a.formMethod) || o.method) !== "get")
                        return;
                    const h = new URL((a == null ? void 0 : a.hasAttribute("formaction")) && (a == null ? void 0 : a.formAction) || o.action);
                    if (_e(h, q))
                        return;
                    const _ = n.target
                    ...
                    y && P.append(y, (a == null ? void 0 : a.getAttribute("value")) ?? ""),
                        ...
                        ce({
                            ...
                            details: {
                                state: {},
                                replaceState: A ?? h.href === location.href
                            },
                            nav_token: {},
                            accepted: () => { }
                            ,
        ...
        ,
        _hydrate: async ({ status: e = 200, error: s, node_ids: n, params: o, route: a, data: c, form: h }) => {
            N = !0;
            ...
            try {
                const v = n.map(async (P, y) => {
                    const w = c[y];
                    return w != null && w.uses && (w.uses = Ye(w.uses)),
                        ue({
                            loader: t.nodes[P],
                            url: _,
                            params: o,
...
async function He(t, r) {
    ...
    if ((p = i.headers.get("content-type")) != null && p.includes("text/html") && await K(t),
        !i.ok)
...
`);
                ...
                if (d = d.slice(D + 1),
                    ...
                    return u(U);
                if (U.type === "data")
                    (N = U.nodes) == null || N.forEach(C => {
                        (C == null ? void 0 : C.type) === "data" && (C.uses = Ye(C.uses),
                            C.data = m(C.data))
                    }
                    ),
                        u(U);
    ...
    if (t)
        ...
        if (i && i.type !== "None") {
            ...
            for (let u = 0; u < i.rangeCount; u += 1)
                p.push(i.getRangeAt(u));
            setTimeout(() => {
                if (i.rangeCount === p.length) {
                    for (let u = 0; u < i.rangeCount; u += 1) {
                        const E = p[u]
                            , l = i.getRangeAt(u);
                        if (E.commonAncestorContainer !== l.commonAncestorContainer || E.startContainer !== l.startContainer || E.endContainer !== l.endContainer || E.startOffset !== l.startOffset || E.endOffset !== l.endOffset)
                            return
                    }
                    i.removeAllRanges()
                }
